...
import { generateProgressAnalysis } from '@/lib/ai/chains/progress-analysis';

/**
 * POST /api/ai/progress-analysis
 *
 * Generate an AI analysis of a developer's progress updates on a task.
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const { task, progressUpdates, assigneeName } = body;
    ...
    const inputText = JSON.stringify({ task, progressUpdates, assigneeName });
    const inputTokens = estimateTokens(inputText);

    // Generate the analysis stream
    const stream = await generateProgressAnalysis({
      task,
